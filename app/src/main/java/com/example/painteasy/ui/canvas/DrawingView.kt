...
class DrawingView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    fun saveBitmap(bitmap: Bitmap): String {
        ...
        val fileName = "PaintEasy_${System.currentTimeMillis() / 1000}.png"
    ...
    fun saveBitmapJpg(bitmap: Bitmap): String {
        val resolver = context.contentResolver
        val fileName = "PaintEasy_${System.currentTimeMillis() / 1000}.jpeg"
        ...
        val uri: Uri? = resolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
        if (uri != null) {
            saveDrawingToStreamJpg(bitmap, resolver.openOutputStream(uri))
            context.contentResolver.update(uri, values, null, null)
            result = uri.toString()
        }
    ...
    fun getBitmapJpg(view: View): Bitmap {
        val bitmap = Bitmap.createBitmap(view.width, view.height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(bitmap)
        val bgDrawing = view.background
    ...
    private fun saveDrawingToStreamJpg(bitmap: Bitmap, outputStream: OutputStream?) {
        if (outputStream != null) {
            try {
