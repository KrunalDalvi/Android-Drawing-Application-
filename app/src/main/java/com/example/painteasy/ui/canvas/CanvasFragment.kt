...
import com.example.painteasy.databinding.FragmentCanvasBinding
import com.example.painteasy.utils.Constants
import com.example.painteasy.utils.Constants.STORAGE_PERMISSION_CODE
import com.example.painteasy.utils.shareText
import com.example.painteasy.utils.snackBarMsg
...
//        ) || super.onOptionsItemSelected(item)
    ...
    private fun saveJpg() {
        when {
            isReadStorageAllowed() -> saveBitmapJpg()
            else -> requestStoragePermission()
        }
    }
    ...
    private fun saveBitmapJpg() {
        viewLifecycleOwner.lifecycleScope.launch {
            binding.drawingView.saveBitmapJpg(binding.drawingView.getBitmapJpg(flDrawingViewContainer))
            snackBarMsg(requireView(), getString(R.string.drawing_saved))
        }
    }
